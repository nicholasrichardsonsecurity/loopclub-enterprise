...
33. **CI GitHub Actions do backend (27/06/2026):** Workflow criado em `.github/workflows/ci.yml`. actions/checkout@v5, actions/setup-node@v5, Node 24, cache npm. Etapas: npm ci, prisma generate, npm test (--runInBand), npm run build. Executado com sucesso no runner Linux (19 testes aprovados, build OK). `DATABASE_URL` fictícia para geração do Prisma Client. Permissão mínima (`contents: read`). Nenhum serviço PostgreSQL.
34. **Infraestrutura de testes e2e (28/06/2026):** Supertest, jest.e2e.config.cjs, test/tsconfig.e2e.json. Seed e2e exclusivo com 9 usuários, 3 empresas e 8 vínculos. Proteção destrutiva: validateTestEnvironment() valida NODE_ENV, DATABASE_URL_TEST, sufixo do banco, host, conflito com banco de desenvolvimento. resetTestDatabase() com TRUNCATE CASCADE. Helper de login (auth-e2e.ts) e assertions (assertions-e2e.ts). 24 testes: 9 segurança + 3 smoke + 12 cenários HTTP. Execução serial com --runInBand. `cross-env`, `supertest` e `@types/supertest` adicionados.
35. **CI com PostgreSQL service container (28/06/2026):** CI atualizada para incluir PostgreSQL 16 Alpine como service container nativo com health check. Etapa de e2e adicionada após o build. DATABASE_URL placeholder fictício; DATABASE_URL_TEST aponta para o banco efêmero. Secrets: CI_POSTGRES_PASSWORD, CI_E2E_TEST_PASSWORD, CI_JWT_SECRET. Timeout aumentado para 15 minutos.
